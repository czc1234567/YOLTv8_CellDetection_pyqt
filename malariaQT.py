...
class malariaThread(QThread):
    ...
    def run(self) -> None:
        ...
        for file in sorted_files:
            cell_info_dict = {}  # 存储细胞信息的字典
            px_info_dict = {}  # 存储疟原虫信息的字典
            file_index += 1  # 文件索引递增
            self.pic = file_index  # 更新当前处理的图片索引

            key = 0  # 初始化键
            imgpath = os.path.join(self.batch_path, file)  # 构建图片完整路径
            matbgr = cvimread(imgpath)  # 读取图片
            matyolo = prefunc(matbgr)  # 对图片进行预处理
            thinkness_res = self.malariathicknessClassifier.infer(matbgr)  # 厚度分类器推断

            # 判断是薄片还是厚片
            if self.thinkness == "thin":
                cell_pred = self.malariaCellDetecter.infer(matyolo, device=DEVICE)  # 细胞检测器推断
                # 检测细胞
                for xywh, class_, conf in zip(cell_pred.boxes.xywh,
                                              cell_pred.boxes.cls,
                                              cell_pred.boxes.conf):
                    cell_info_dict[key] = {}  # 初始化字典存储当前细胞信息
                    cx, cy, w, h = map(int, xywh)  # 提取坐标和宽高
                    x = int(cx - w / 2)  # 计算左上角x坐标
                    y = int(cy - h / 2)  # 计算左上角y坐标
                    cell_info_dict[key]['类型'] = "cell"  # 设置类型为细胞
                    cell_info_dict[key]['置信度'] = conf.item()  # 设置置信度
                    cell_info_dict[key]['位置矩形'] = (x, y, w, h)  # 设置位置矩形
                    key += 1  # 递增键

            # 检测疟原虫
            if self.thinkness == "thin":
                curent_model = self.thinDetecter  # 获取当前模型
                px_pred, conf_number, file_number = curent_model.infer(imgpath)  # 模型推断

                conf_numbers += conf_number  # 更新总置信度数量
                file_numbers += file_number  # 更新总文件数量
                ...
                for xywh, class_, conf in zip(px_pred.boxes.xywh,
                    ...
                    cx, cy, w, h = map(int, xywh)  # 提取坐标和宽高
                    if int(class_) == 0:
                        px_info_dict[key] = {}  # 初始化字典存储当前疟原虫信息
                        class_wbc = px_pred.names[int(class_)].lower()  # 获取类别名称并转换为小写
                        x = int(cx - w / 2)  # 计算左上角x坐标
                        y = int(cy - h / 2)  # 计算左上角y坐标
                        px_info_dict[key]['类型'] = class_wbc  # 设置类型
                        px_info_dict[key]['置信度'] = conf.item()  # 设置置信度
                        px_info_dict[key]['位置矩形'] = (x, y, w, h)  # 设置位置矩形
                        key += 1
                        continue  # 继续下一个循环
                    # 限定检测框的尺寸范围，防止检测到过小的物体同时滤除长宽比不合理的物体
                    if w < 2 * h and h < 2 * w and w * h < 150000 and w * h > 25600:
                        px_info_dict[key] = {}  # 初始化字典存储当前疟原虫信息
                        class_ = px_pred.names[int(class_)].lower()  # 获取类别名称并转换为小写
                ...
                # 构建批次信息字典
                batch_info[file_index] = {
                    "imgseed": file_index,  # 图片索引
                    "thinkness": thinkness_res[0],  # 厚度结果
                    "thinkness_conf": thinkness_res[1],  # 厚度置信度
                    "info": {}  # 存储检测信息
                }
            ...
            if check_path_not_empty(path):
                ...
                for result in results:
                    ...
                    for top in result.probs.top5:
                        ...
                        cv2.putText(result.orig_img, f"{result_name} {conf:.2f}", (10, 20 * n),
                ...
                if PO_percent >= PV_percent:
                    PO_percent += POPV_percent
                else:
                    PV_percent += POPV_percent
    ...
    def load_weight(self, thin_weight, thick_weight):
        ...
        self.thickDetecter = loadDetecter(thick_weight, 0.25, 0.45)
    ...
    def load_thick_weight(self, path):
        self.thickDetecter = loadDetecter(path, 0.25, 0.45)
